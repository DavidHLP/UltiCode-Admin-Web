        ...
        <TabView>
                    ...
                    <template #start>
                        <div class="grid gap-3 grid-cols-1 md:grid-cols-3 w-full md:w-auto">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <InputNumber v-model="nodeRefreshMs" :min="5000" :step="5000" inputId="node-refresh" suffix=" ms" placeholder="刷新间隔" style="width: 10rem" />
                            </div>
                        </div>
                    </template>
                    <template #end>
                            ...
                            <div v-for="node in slotProps.items" :key="node.id" class="">
                                <Panel :header="node.name" toggleable class="rounded-2xl">
                                    <div class="flex items-center justify-between mb-3">
                                        <div class="text-sm text-surface-500 dark:text-surface-300">最后心跳：{{ formatDate(node.lastHeartbeat) }}</div>
                                    ...
                                    <div class="grid grid-cols-4 gap-2 text-center mb-3">
                                        <div>
                                            ...
                                            <span class="text-lg font-semibold">{{ node.metrics?.queuedJobs ?? 0 }}</span>
                                        </div>
                                        <div>
                                            <span class="block text-xs text-muted-color">运行</span>
                                            <span class="text-lg font-semibold">{{ node.metrics?.runningJobs ?? 0 }}</span>
                                        </div>
                                        <div>
                                            <span class="block text-xs text-red-500">失败</span>
                                            <span class="text-lg font-semibold">{{ node.metrics?.failedJobs ?? 0 }}</span>
                                        </div>
                                        <div>
                                            <span class="block text-xs text-muted-color">1h 完成</span>
                                            <span class="text-lg font-semibold">{{ node.metrics?.finishedLastHour ?? 0 }}</span>
                                        </div>
                                    </div>

                                    <Divider class="my-2" />
                                    <div v-if="runtimeEntries(node).length" class="space-y-1 text-sm text-muted-color">
                                        <div v-for="entry in runtimeEntries(node)" :key="entry" class="flex items-center gap-2">
            ...
            <TabPanel header="任务" value="">
                <Fieldset legend="筛选与控制" toggleable class="rounded-2xl">
                    <div class="flex flex-wrap items-end justify-between gap-3">
                        <div class="grid gap-3 grid-cols-1 md:grid-cols-3 lg:grid-cols-4 w-full">
                            ...
                            <InputText v-model="jobFilters.keyword" placeholder="按用户或题目关键字" @keyup.enter="onSearchJobs" />
                        </div>
                        <div class="flex items-center gap-3 flex-wrap">
                            <ToggleButton v-model="jobAutoRefresh" onLabel="自动刷新" offLabel="自动刷新" :onIcon="'pi pi-sync'" :offIcon="'pi pi-sync'" />
                            <InputNumber v-model="jobRefreshMs" :min="5000" :step="5000" suffix=" ms" placeholder="刷新间隔" style="width: 10rem" />
                ...
                >
                    ...
                    <template #empty>
                        <div class="py-6 text-center text-surface-500 dark:text-surface-300">暂无任务</div>
                    </template>
                </DataTable>
            </TabPanel>
        </TabView>
            ...
            <template v-else-if="jobDetail">
                ...
                <Panel header="提交信息" toggleable class="rounded-2xl mb-4">
                    <div v-if="jobDetail.job.submission" class="grid sm:grid-cols-2 lg:grid-cols-3 gap-3 text-sm">
                        ...
                        <div><span class="text-muted-color mr-2">耗时：</span>{{ jobDetail.job.submission.timeMs ?? '-' }} ms</div>
                        <div><span class="text-muted-color mr-2">内存：</span>{{ jobDetail.job.submission.memoryKb ?? '-' }} KB</div>
                    </div>
                ...
                <Panel header="测试结果" toggleable class="rounded-2xl mb-4">
                    <DataTable :value="jobDetail.tests" dataKey="id" :rows="5" :paginator="true" responsiveLayout="scroll">
                        ...
                        <Column field="groupName" header="测试组" style="min-width: 8rem">
                            <template #body="{ data }">
                                <Tag v-if="data.groupName" :value="data.groupName" :severity="data.sampleGroup ? 'info' : 'secondary'" />
                                <span v-else>-</span>
                            </template>
                        </Column>
                ...
                <Panel header="日志与附件" toggleable class="rounded-2xl">
                    ...
                    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                        <div v-for="artifact in jobDetail.artifacts" :key="artifact.id" class="border rounded-xl p-3">
                            ...
                            <div class="text-sm text-muted-color mb-2">文件 ID：{{ artifact.fileId ?? '-' }} ｜ 大小：{{ artifact.sizeBytes ?? '-' }} bytes</div>
                            <div class="flex flex-wrap gap-2">
                                <Button label="复制路径" icon="pi pi-copy" size="small" outlined @click="copyText(artifact.storageKey || '', '存储路径')" />
                                <Button label="复制哈希" icon="pi pi-clone" size="small" outlined @click="copyText(artifact.sha256 || '', 'SHA256')" />
                            </div>
                        </div>
                    </div>
                </Panel>
