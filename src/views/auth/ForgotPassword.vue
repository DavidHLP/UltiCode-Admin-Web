            ...
            <div style="border-radius: 56px; padding: 0.3rem; background: linear-gradient(180deg, var(--primary-color) 10%, rgba(33, 150, 243, 0) 30%)">
                <div class="w-full bg-surface-0 dark:bg-surface-900 py-20 px-8 sm:px-20" style="border-radius: 53px">
                    <div class="text-center mb-8 flex flex-col items-center gap-4">
                        ...
                        <span class="text-muted-color font-medium text-base max-w-[22rem]"> 输入您绑定的邮箱地址，我们将发送一封带有重置链接的邮件。 </span>
                    </div>

                    <form @submit.prevent="onSubmit" class="flex flex-col">
                        <label for="email" class="block text-surface-900 dark:text-surface-0 text-xl font-medium mb-2">邮箱</label>
                        <div class="mb-6 flex flex-col gap-2">
                            <InputText id="email" v-model="form.email" type="email" placeholder="邮箱地址" class="w-full md:w-[30rem]" :invalid="!!errors.email" autocomplete="username" :disabled="isSubmitting" />
                            <InlineMessage v-if="errors.email" severity="error">{{ errors.email }}</InlineMessage>
                        </div>

                        <Message v-if="isSubmitted" severity="success" class="mb-6"> 如果您提供的邮箱已注册，我们会在几分钟内发送重置密码邮件。 </Message>
